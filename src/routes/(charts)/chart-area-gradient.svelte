		...
		<ChartContainer>
			...
			>
				<!-- <svelte:fragment slot="marks">
					{@const primaryColor = chartConfig.desktop.color}
					{@const secondColor = `${primaryColor} / 10%)`}
					<LinearGradient id="fillDesktop" stops={[primaryColor, secondColor]} vertical let:url>
						<Area
							curve={curveNatural}
							line={{ class: `stroke-1 stroke-[${primaryColor}]` }}
							fill={url}
							tweened
						/>
					</LinearGradient>
				...
				<!-- <svelte:fragment slot="marks">
					{#each Object.entries(chartData) as [_, { mobile, desktop }]}
						{@const primaryColor = chartConfig.desktop.color}
						{@const secondColor = `${primaryColor} / 10%)`}
						<LinearGradient id="fill-desktop" stops={[primaryColor, secondColor]} vertical let:url>
							<Area
								curve={curveNatural}
								line={{ class: `stroke-1 stroke-[${primaryColor}]` }}
								fill={url}
						...
						<LinearGradient id="fill-mobile" stops={[primaryColor, secondColor]} vertical let:url>
							<Area
								curve={curveNatural}
								line={{ class: `stroke-1 stroke-[${primaryColor}]` }}
								fill={url}
